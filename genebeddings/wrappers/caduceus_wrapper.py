...
class CaduceusWrapperOld(BaseWrapper):
    """
    Caduceus wrapper with standardized API.

    Notes
    -----
    * Caduceus models on HF are standard HF masked-LM checkpoints with a tokenizer.
    * Many Caduceus tokenizers are **character-level** (k=1). We still infer k generically.
    * For long sequences, keep inputs <= model_max_length (including specials).
    """

    def __init__(
        self,
        model_id: str = "kuleshov-group/caduceus-ph_seqlen-131k_d_model-256_n_layer-16",
        *,
        device: Optional[str] = None,
        dtype: torch.dtype = torch.float32,
        trust_remote_code: bool = True,
        attn_impl: str = "eager",   # be safe across backends
        ...
        # use the base encoder for embeddings (varies by model impl)
        self.backbone = getattr(self.model, "base_model", None)
        if self.backbone is None:
            self.backbone = getattr(self.model, "model", self.model)
    
        # # We use the MLM model for both logits and embeddings (last_hidden_state)
        # self.model = AutoModelForMaskedLM.from_pretrained(
        #     model_id,
        #     torch_dtype=dtype,
        #     attn_implementation=attn_impl,
        ...
        # self.backbone = self.model  # last_hidden_state available
        self.k = self._infer_k_nonoverlap()
        self.mask_id = getattr(self.tokenizer, "mask_token_id", None)
        if self.mask_id is None:
            raise ValueError("Tokenizer has no [MASK] token; Caduceus MLM requires a mask token.")
    ...
    def predict_nucleotides(
        ...
        *,
        ...
        """
        Predict nucleotide probabilities at specified base positions.
        For Caduceus with k=1, this maps directly. For k>1, we use pattern matching.
        """
        token_ids = self.tokens_from_seq(seq)
        ...
        if return_dict:
            ...
            for pos in positions:
                ...
                token_idx = pos // self.k
                rel_offset = pos % self.k

                if token_idx >= len(token_ids):
                    raise IndexError(f"Token index {token_idx} out of range")

                # Get masked logits at this token
                logits = self.get_masked_position_logits(
                    token_ids_no_specials=token_ids,
                    token_position_of_interest=token_idx,
                    return_probs=False,
                )

                # If k=1, we can directly extract base probs
                if self.k == 1:
                    # Character-level: logits should have base tokens
                    # Find token IDs for A, C, G, T
                    ...
                    base_probs = {}
                    for b in BASES:
                        tid = base_ids.get(b, -1)
                        if tid >= 0 and tid < len(probs_tensor):
                            base_probs[b] = float(probs_tensor[tid].item())
                        else:
                            base_probs[b] = 0.25  # uniform fallback

                    # Normalize
                    total = sum(base_probs.values())
                    if total > 0:
                        base_probs = {b: v/total for b, v in base_probs.items()}
                else:
                    # k > 1: use pattern matching
                    pattern = 'N' * self.k
                    pattern = pattern[:rel_offset] + 'N' + pattern[rel_offset+1:]
                    base_probs = self.pattern_matching_filter(
                        logits,
                        target_pattern_with_N=pattern,
                        masked_idx_in_token=rel_offset,
                    )
            ...
            for i, pos in enumerate(positions):
                ...
                if self.k == 1:
                    ...
                    for b in BASES:
                        tid = base_ids.get(b, -1)
                        if tid >= 0 and tid < len(probs_tensor):
                            base_probs_dict[b] = float(probs_tensor[tid].item())
                        else:
                            base_probs_dict[b] = 0.25
                    total = sum(base_probs_dict.values())
                    if total > 0:
                        base_probs_dict = {b: v/total for b, v in base_probs_dict.items()}
                else:
                    pattern = 'N' * self.k
                    pattern = pattern[:rel_offset] + 'N' + pattern[rel_offset+1:]
                    base_probs_dict = self.pattern_matching_filter(
                        logits,
                        target_pattern_with_N=pattern,
                        masked_idx_in_token=rel_offset,
                    )

                results[i] = np.array([base_probs_dict['A'], base_probs_dict['C'],
                                      base_probs_dict['G'], base_probs_dict['T']], dtype=np.float32)
            return results
    ...
    def embed(
        ...
        *,
        ...
        enc = self.tokenizer(
            seq,
            return_tensors="pt",
            return_attention_mask=True,      # <-- ask for it
        ...
        # move to device explicitly (BatchEncoding.to() can be flaky on some versions)
        input_ids = enc["input_ids"].to(self.device)
        
        # RUN WITHOUT attention_mask (Caduceus.forward doesn't accept it)
        out = self.backbone(
            input_ids=input_ids,
        ...
        # get last-layer token embeddings
        last = out.last_hidden_state[0] if hasattr(out, "last_hidden_state") and out.last_hidden_state is not None \
               else out.hidden_states[-1][0]
        
        # pooling: since we didn't pad, mask = ones
        mask = torch.ones(last.shape[0], 1, device=last.device)
        
        if pool == "mean":
            denom = mask.sum(dim=0).clamp(min=1)
            emb = (last * mask).sum(dim=0) / denom
        elif pool == "cls":
            emb = last[0]
        elif pool == "tokens":
            emb = last
        ...
        return emb.detach().cpu().numpy() if return_numpy else emb
    ...
    def pattern_matching_filter(
        ...
        target_pattern_with_N: str,                    # len=k; 'N' as wildcard; may contain multiple 'N'
        *,
        masked_idx_in_token: Optional[int] = None,     # required if pattern has != 1 'N'
        temperature: float = 1.0,
        ...
        if masked_idx_in_token is None:
            ...
            if n_count == 1:
                ...
                raise ValueError(
                    "masked_idx_in_token must be provided when the pattern contains "
                    f"{n_count} 'N' positions (need to know which 'N' is the masked site)."
                )
        ...
        if logits.dim() != 1:
            raise ValueError("logits_vec must be a 1D vector (V,).")
        if temperature <= 0:
            raise ValueError("temperature must be > 0.")
        probs = torch.softmax(logits / temperature, dim=-1).detach().cpu().numpy()

        base_buckets = {b: 0.0 for b in BASES}
        ...
        for tok_str, tok_id in vocab.items():
            s = self._id2dna(tok_id, tok_str, clean)
            if len(s) != self.k:
                continue
            if not token_matches(s, pat):
                continue
            base_at_mask = s[masked_idx_in_token]
            if base_at_mask in base_buckets:
                p = float(probs[tok_id])
                base_buckets[base_at_mask] += p
                if return_debug:
                    debug_pairs.append((s, p))

        total = sum(base_buckets.values())
        if total <= 0:
            result = {b: 0.25 for b in BASES}
            return (result, []) if return_debug else result

        result = {b: v / total for b, v in base_buckets.items()}
        if return_debug:
            debug_pairs.sort(key=lambda x: x[1], reverse=True)
            return result, debug_pairs
        return result
    ...
    def center_token_mapping(
        ...
        *,
        ...
        """
        Build window [query_pos-L+1, query_pos+L+1), make length divisible by k,
        locate the token covering the center base, and expose the k-mer chunk.

        Uses 'N' at the mutation in seq_ref (masked site), other positions unchanged (may include N).
        """
        ...
        seq_ref = seq.clone()
        # mark the masked position with 'N' (your convention)
        seq_ref.apply_mutations([(query_pos, query_ref, "N")], permissive_ref=True)

        seq_s = seq.seq
        seq_ref_s = seq_ref.seq

        # trim right to make divisible by k
        while len(seq_s) % self.k != 0:
            seq_s = seq_s[:-1]
            seq_ref_s = seq_ref_s[:-1]
        ...
        expected_seq_chunk = seq_ref_s[token_pos * self.k : (token_pos + 1) * self.k]
        token_string_clean = re.sub(r"[^ACGTN]", "", tokens[token_pos])
        ...
        return {
            ...
            "corresponding_seq_chunk": expected_seq_chunk,     # contains 'N' at masked site
    ...
    def _infer_k_nonoverlap(self) -> int:
        ...
        for t in ids[:64]:
            ...
            if s and set(s).issubset(DNA_SET):
                return len(s)  # k-mer length
        # Fallback to char-level if we couldn't infer (Caduceus is often k=1)
        return 1
